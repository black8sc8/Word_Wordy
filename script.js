...
const allQuestions = [
    ...
    { 문: "문어는 다리가 많아 🐙", eng: "An octopus has many legs" },
...
// 🔊 [TTS 기능] 전달받은 텍스트를 영어 발음으로 읽어주는 함수
function speakText(text) {
    // 브라우저가 SpeechSynthesis를 지원하는지 확인
    if ('speechSynthesis' in window) {
        // 기존에 읽고 있던 소리가 있다면 취소
        window.speechSynthesis.cancel();

        const utterance = new SpeechSynthesisUtterance(text);
        utterance.lang = 'en-US'; // 미국식 영어 발음
        utterance.rate = 0.85;    // 아이들이 듣기 편하게 속도를 약간 천천히(0.85) 설정
        utterance.pitch = 1.1;    // 목소리 톤을 약간 높여 밝은 느낌 연출
...
function checkAnswer() {
    ...
    if (userAnswer === currentQuestion.eng) {
        showMessage("참 잘했어요! 💯 정답입니다!", true);
        
        // 🎉 정답일 때 전체 완성 문장을 한 번 더 읽어줌
        ...
        setTimeout(() => {
            nextQuestion();
        }, 1500); // 문장 전체 발음을 들을 수 있도록 다음 문제 이동 시간을 1.5초로 살짝 조정
